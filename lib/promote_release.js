...
export default class ReleasePromotion extends Session {
  ...
  async parseDataFromReleaseCommit() {
    ...
    const data = await forceRunAsync('git', ['--no-pager', 'log', '-1', '--pretty=format:%H+%s'], {
      captureStdout: true
    });
    ...
    if (this.date !== systemDate) {
      cli.warn(`The release date (${this.date}) does not match the system date for today (${systemDate}).`);
      if (!await cli.prompt('Do you want to proceed?', { defaultAnswer: false })) {
        throw new Error('Aborted');
      }
    }
