...
export default class ReleasePromotion extends Session {
  ...
  async promote() {
    ...
    let localCloseIsClean = true;
    const currentHEAD = await forceRunAsync('git', ['rev-parse', 'HEAD'], { captureStdout: true });
    if (currentHEAD.trim() !== releaseCommitSha) {
      cli.warn('Current HEAD is not the release commit');
      localCloseIsClean = false;
    }
    try {
    ...
    if (!localCloseIsClean) {
      if (await cli.prompt('Should we reset the local HEAD to be the release proposal?')) {
        ...
        await forceRunAsync('git', ['reset', releaseCommitSha, '--hard']);
      } else {
  ...
  async parseDataFromReleaseCommit() {
    ...
    if (this.date !== systemDate) {
      cli.warn(
        `The release date (${this.date}) does not match the system date for today (${systemDate}).`
      );
    ...
    this.version = releaseCommitMessage.slice(20, releaseCommitMessage.indexOf(' ', 20));
    const version = semver.parse(this.version);
    if (!version) {
      cli.error(`Release commit contains invalid semantic version: ${this.version}`);
      throw new Error('Aborted');
    }
    ...
    const changeLogDiff = await forceRunAsync('git', [
      ...
      '--',
      `doc/changelogs/CHANGELOG_V${version.major}.md`
    ], { captureStdout: true });
