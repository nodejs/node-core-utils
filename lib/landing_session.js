...
class LandingSession extends Session {
  ...
  async continue() {
    ...
    if (this.rebaseInProgress()) {
      cli.log(`Running \`git rebase --continue\`..`);
      return this.rebaseContinue();
    }
  ...
  async rebaseContinue() {
    await runAsync('git', ['rebase', '--continue']);
  }

  async rebase() {
    const { upstream, branch, cli } = this;
    await runAsync('git', [
      'rebase', '-i', '--autosquash', `${upstream}/${branch}`
    ]);
    cli.ok('Rebased current branch');
    return runSync('git', ['add', '.']).trim();
  }
