...
export default class Request {
  ...
  async * stream(url, options = {}) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Unable to stream ${url}: ${res.status} ${res.statusText}`);
    }
    // Undici decompresses HTTP content encodings as it reads. Returning early
    // from this iterator also cancels the response body and its HTTP download.
    yield * res.body;
  }
  ...
  async * getPullRequestFiles({ owner, repo, prid }) {
    ...
    for (;;) {
      ...
      if (!Array.isArray(batch)) {
        throw new Error('Unable to retrieve pull request files');
      }
      if (batch.length === 0) {
        break;
      }
