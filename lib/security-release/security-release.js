...
export async function checkoutOnSecurityReleaseBranch(cli, repository) {
  ...
  if (currentBranch !== NEXT_SECURITY_RELEASE_BRANCH) {
    const refs = new Set(runSync('git', [
      'for-each-ref', '--format=%(refname)',
      `refs/heads/${NEXT_SECURITY_RELEASE_BRANCH}`,
      `refs/remotes/origin/${NEXT_SECURITY_RELEASE_BRANCH}`
    ]).trim().split('\n'));
    let args;
    if (refs.has(`refs/heads/${NEXT_SECURITY_RELEASE_BRANCH}`)) {
      args = ['checkout', NEXT_SECURITY_RELEASE_BRANCH];
    } else if (refs.has(`refs/remotes/origin/${NEXT_SECURITY_RELEASE_BRANCH}`)) {
      args = ['checkout', '-b', NEXT_SECURITY_RELEASE_BRANCH,
        '--track', `origin/${NEXT_SECURITY_RELEASE_BRANCH}`];
    } else {
      args = ['checkout', '-b', NEXT_SECURITY_RELEASE_BRANCH];
    }
    await runSecurityGitCommand(
      cli,
      args,
      `This switches to ${NEXT_SECURITY_RELEASE_BRANCH} without resetting an existing branch.`
    );
...
export async function commitAndPushVulnerabilitiesJSON(
  ...
  const stagedFiles = runSync('git', ['diff', '--cached', '--name-only', '-z'])
    .split('\0').filter(Boolean);
  const unrelated = stagedFiles.filter((file) => {
    const absolute = path.resolve(file);
    return !files.some((target) =>
      absolute === target || absolute.startsWith(`${target}${path.sep}`));
  });
  if (unrelated.length) {
    throw new Error(
      'Unrelated staged changes would be included in the security release commit: ' +
      unrelated.join(', ') + '. Commit or unstage them before continuing.'
    );
  }
...
// `dependencies` maps a dependency name to its update, or to a list of updates
// when the release lines got different ones. Flatten it into a list of updates
// that each know their name.
export function getDependencyUpdates(dependencies) {
  return Object.entries(dependencies ?? {}).flatMap(([name, update]) =>
    (Array.isArray(update) ? update : [update]).map((entry) => ({ ...entry, name })));
}
