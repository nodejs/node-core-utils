...
const RUN_TREE = 'actions[causes[upstreamBuild,upstreamProject]]';
...
class TestBuild extends Job {
  ...
  setBuildData({result, changeSet, actions, timestamp}) {
...
class PRBuild extends TestBuild {
  ...
  display() {
    this.displayHeader();
    if (this.commitBuild) {
      this.commitBuild.displayBuilds();
    } else {
      this.cli.log('Failed to trigger node-test-commit');
    }
  }
  ...
  formatAsMarkdown() {
    if (this.commitBuild) {
      ...
      return 'Failed to trigger node-test-commit';
    }
  }
...
class NormalBuild extends Job {
  ...
  async getResults() {
    ...
    const causes = await Promise.all(tests.map(run => run.getCause()));
...
class TestRun extends Job {
  ...
  async getResults() {
    ...
    const parser = new CIParser(this.consoleUIUrl, text);
    const results = parser.parse();
    if (results) {
      this.failures = results;
      return results;
