...
export class PRBuild extends TestBuild {
  ...
  async getResults() {
    ...
    // Jenkins can retain the result after the nested build is no longer available.
    if (!commitBuild.build) {
      this.commitBuild.setBuildData(buildData);
      this.failures = result === statusType.SUCCESS
        ? []
        : [new NCUFailure({ url: this.commitBuild.jobUrl }, 'Build data is unavailable')];
      this.commitBuild.failures = this.failures;
      return { result, builds: this.builds, failures: this.failures };
    }
