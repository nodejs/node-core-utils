...
const fatalPattern = /fatal: .+/g;

// Shared by the whole-log parser and the streaming file scanner.
export const FAILURE_MARKERS = {
  ...
  cpp: /\[ {2}FAILED {2}\]/,
  git: [{
    start: /Changes not staged for commit:/,
    end: /no changes added to commit/
  }, {
    start: /error: Your local changes to the following files/,
    end: /Failed to merge in the changes./
  }]
};

const tapPattern = new RegExp(
  `${FAILURE_MARKERS.tap.start.source}[\\s\\S]+?${FAILURE_MARKERS.tap.end.source}`, 'mg');
...
const FAILURE_FILTERS = [{
  ...
  filter(ctx, text) {
    const matches = text.match(tapPattern);
    if (!matches) {
      return null;
    }
    const nonFlaky = matches.filter((m) => !m.includes(FAILURE_MARKERS.tap.todo));
    if (!nonFlaky.length) {
      return null;
    }
  ...
  Failure: CCTestFailure,
  patterns: [{
    pattern: new RegExp(`${FAILURE_MARKERS.cpp.source}.+`, 'g'),
  ...
  Failure: GitFailure,
  patterns: [...FAILURE_MARKERS.git.map(({ start, end }) => ({
    pattern: new RegExp(`${start.source}[\\s\\S]+${end.source}`, 'mg'),
  ...
  filter(ctx, text) {
    const matches = text.match(fatalPattern);
    if (!matches) {
      return null;
    }
...
// Share recognition patterns with streaming consumers without exposing the
// parser's constructors, filter priority, or context-selection machinery.
export const FAILURE_PATTERNS = {
  infrastructure: FAILURE_FILTERS
    .filter(({ Failure }) => Failure === InfraFailure)
    .flatMap(({ patterns }) => patterns.map(({ pattern }) => pattern)),
  diagnostic: [
    ...FAILURE_FILTERS
      .filter(({ Failure }) => Failure && Failure !== InfraFailure && Failure !== CCTestFailure)
      .flatMap(({ patterns }) => patterns.map(({ pattern }) => pattern)),
    fatalPattern
  ]
};
...
export default class CIFailureParser {
  ...
  parse() {
    ...
    for (const { filter, Failure, patterns } of FAILURE_FILTERS) {
      const result = filter
        ? filter(this.ctx, text)
        : failureMatcher(Failure, patterns, this.ctx, text);
