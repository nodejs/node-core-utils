...
export function getCIActionAdvice(pr, action) {
  const command = `ncu-ci ${action} ${getPrURL(pr)}`;
  if (action === 'run') {
    return 'Check the existing CI run in Jenkins and rebase the PR if needed. ' +
      `To start a new CI run manually: ${command}`;
  }
  if (pr.owner === 'nodejs' && pr.repo === 'node') {
    return `Resume CI by adding the "resume-ci" label to the PR, or run: ${command}`;
  }
  return `Resume CI with: ${command}`;
}
