...
export class RunPRJob {
  ...
  async start() {
    ...
    if (checkForDuplicates) {
      ...
      try {
        if (jobid) {
          buildData = await new PRBuild(cli, request, jobid, undefined, RESUME_TREE)
            .getBuildData();
        }
      } catch (err) {
      ...
      const matchingSHA = buildData?.actions?.some(action =>
        action.parameters?.some(parameter =>
          parameter.name === 'COMMIT_SHA_CHECK' && parameter.value === certifySafe));
      if (matchingSHA) {
        cli.info('Existing CI run found: ' + link);
        let advice = 'Check the existing CI run in Jenkins before retrying.';
        if (getApprovedSHA(buildData) === certifySafe && buildData.building === false &&
            ['FAILURE', 'ABORTED'].includes(buildData.result)) {
          try {
            const { build } = await findResumableBuild(cli, request, {
              jobid,
              owner: this.owner,
              repo: this.repo,
              prid: this.prid
            }, buildData);
            advice = getCIActionAdvice(this, build ? 'resume' : 'run');
          } catch (err) {
            cli.error(`Could not check whether existing CI run ${link} can be resumed: ` +
              err.message);
            cli.error('Refusing to start a potentially duplicate CI job. ' +
              'Retry after checking Jenkins access.');
            return false;
          }
        } else if (buildData.building === true) {
          advice = 'The existing CI run is still in progress.';
        } else if (buildData.result === 'SUCCESS') {
          advice = 'CI has already succeeded for this commit.';
        }
        cli.error(`Refusing to start a potentially duplicate CI job. ${advice}`);
        return false;
      }
    }
