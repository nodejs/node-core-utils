...
import { FAILURE_PATTERNS } from './ci_failure_parser.js';
...
async function * failureLines(windows, matchFile) {
  ...
  for await (const { text, lineStart, lineEnd } of windows) {
    ...
    line.cpp ||= /\[ {2}FAILED {2}\]/.test(text);
    line.tapStart ||= lineStart && /^not ok \d+/.test(text);
    line.tapEnd ||= lineEnd && / {2}\.\.\.\n$/.test(text);
    line.todo ||= text.includes('# TODO :');
    line.gitStart ||= text.includes('Changes not staged for commit:') ||
      text.includes('error: Your local changes to the following files');
    line.gitEnd ||= text.includes('no changes added to commit') ||
