...
export default class SecurityBlog {
  ...
  async getReportsTemplate(content) {
    ...
    for (const report of reports) {
      ...
      if (!cveId) {
        this.cli.error(`CVE ID for vulnerability https://hackerone.com/reports/${report.id} not found`);
        process.exit(1);
      }
 ...
 in active release lines: ${releaseLines}\n\n`;
      if (!report.patchAuthors) {
        this.cli.error(`Missing patch author for vulnerability https://hackerone.com/reports/${report.id} ${report.title}`);
        process.exit(1);
      }
