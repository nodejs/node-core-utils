...
export default class SecurityBlog extends SecurityRelease {
  ...
  async createPreRelease(nodejsOrgFolder) {
    ...
    await this.updateWebsiteBanner(site, {
    ...
    await writeSecurityFile(
      ...
      file,
      preRelease,
      'This writes the pre-release vulnerability announcement post.'
    );
  ...
  async updateAnnouncementLink(link) {
    ...
    await writeSecurityFile(
      ...
      'This writes the announcement link to vulnerabilities.json.'
    );
  ...
  async createPostRelease(nodejsOrgFolder) {
    ...
    await writeSecurityFile(
      ...
      preReleasePath,
      updatedContent,
      'This writes the post-release vulnerability announcement post.'
    );
  ...
  async updateWebsiteBanner(siteJsonPath, content) {
    ...
    await writeSecurityFile(
      ...
      'This writes the updated security release website banner.'
    );
  }
 ...
 and thank you ${report.patchAuthors.join(' and ')} for fixing it.\n\n`;
  ...
  getDependencyUpdatesTemplate(dependencyUpdates) {
    const updates = getDependencyUpdates(dependencyUpdates);
    if (updates.length === 0) return '';
    let template = '\nThis security release includes the following dependency' +
      ' updates to address public vulnerabilities:\n';
    for (const update of updates) {
      const title = this.formatDependencyUpdateTitle(update) || update.name;
      const releaseLines = getAffectedVersionLines(update.affectedVersions);
      const formattedVersions = this.formatDependencyVersions(update.versions);
  ...
  formatDependencyVersions(versions) {
    ...
    // The current schema carries one entry per release line, so the same
    // version repeats when every line gets the same update.
