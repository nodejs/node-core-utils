...
function checkAndParseArgs(args) {
  ...
  if (!isNaN(args.identifier)) {
    return {
      ...
      prid: +args.identifier
    };
  }
  const match = args.identifier.match(PR_RE);
  if (match !== null) {
    if (typeof args.r !== 'undefined' || typeof args.o !== 'undefined') {
      throw new Error(
        'Cannot pass second or third argument ' +
        'when url given as first argument.');
    }
    return {
      owner: `${match[1]}`,
      repo: `${match[2]}`,
