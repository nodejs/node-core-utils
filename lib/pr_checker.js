...
class PRChecker {
  ...
  checkCI() {
    ...
    for (const [type, ci] of ciMap) {
      ...
      if (!lastCiDate || lastCiDate < ci.date) {
        lastCiDate = ci.date;
      }
    }

    const commitsAfterCi = [];
    if (lastCiDate) {
      commits.forEach((c) => {
        const commit = c.commit;
        if (commit.committedDate > lastCiDate) {
          commitsAfterCi.push(commit);
        }
      });
    }

    if (commitsAfterCi.length > 0) {
      logger.warn('Commits pushed after the last CI run:');
      commitsAfterCi.forEach((c) => {
