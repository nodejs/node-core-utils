...
const {
  ERROR, EXPECTED, PENDING, FAILURE, SUCCESS
} = require('./pr_status');
    ...
    if (lastCI) {
      ...
      if (commits.length > 0) {
        ...
        switch (status.state) {
          case EXPECTED:
          case PENDING:
            cli.warn('CI test results are still pending');
            break;
          case SUCCESS:
            cli.info('All CI tests are successful');
            break;
          case FAILURE:
            cli.warn('Some or all CI tests seems to be failing or flasky');
            break;
          case ERROR:
            cli.error('CI testing seems to be error out');
            break;
        }
      }
    }

    this.CIStatus = _status;
    return _status;
  }
